...
void Graphics::loadBackgroundImg() {
  ...
  _images.push_back(
    background);         // first element is the original background
  _images.push_back(
...
void Graphics::drawTrafficObjects() {
  ...
  for (auto it : _trafficObjects) {
    ...
    if (it->getType() == ObjectType::objectIntersection) {
      ...
      std::shared_ptr<Intersection> intersection = std::dynamic_pointer_cast<Intersection>(
        it);

      // set color according to traffic light and draw the intersection as a circle
      cv::Scalar trafficLightColor =
        intersection->trafficLightIsGreen() == true ? cv::Scalar(0, 255, 0)
                                                    : cv::Scalar(0, 0, 255);
